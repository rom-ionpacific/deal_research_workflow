...
@mcp_registry.tool(
    ...
    (
        ...
        "eventualities for a company's strategies. Resolves the company "
        "(from org_ids, or from deal_id if the user named a deal) and "
        "returns: its current base value (eventualities are exit MULTIPLES "
        "on this, not dollar figures); every is_reviewed=TRUE active "
        "strategy with its probability and, for each, its existing "
        "eventuality rows if any (so you can see what's already mapped vs "
        "still pending, or being revisited); and the global "
        "exit_outcome_prior -- the 4-tier baseline (failure/downside/base/"
        "upside with probability + exit multiple + years-to-exit) to WEIGH "
        "against company- and comp-specific evidence, not to copy "
        "wholesale unless nothing better is available. A strategy that "
        "isn't is_reviewed yet cannot have eventualities set -- point the "
        "user to finalize_strategy_agreement first if they ask for one. "
        "Read-only."
    ),
