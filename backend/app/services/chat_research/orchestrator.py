...
_OPENER_INSTRUCTIONS: dict[str, str] = {
    ...
    "entity_select": (
        "## OPENER (when the user's message is exactly `__opener__`)\n"
        "Run the relevance-rundown routine. Goal: give the user a "
        "concrete, ready-to-action picture of what's available + a "
        "specific recommendation they can confirm or modify.\n"
        "\n"
        ...
        "\n"
        "3. Make a SPECIFIC initial recommendation -- not just a "
        "   sentence, an actual plan listing which buckets per entity "
        "   type. Example:\n"
        "     \"I'd start with:\n"
        "       - documents: target + portfolio_company (30 of 38)\n"
        "       - email threads: investor + other (12 of 15)\n"
        "       - calendar events: all 3\n"
        "       - slack: nothing to include\n"
        "     Skipping the 8 comparable / mentioned documents and 3 "
        "     mentioned emails since those are peripheral references "
        "     rather than analyses of the company.\"\n"
        "\n"
        "4. Ask: \"Want me to go ahead with that selection, or would "
        "   you like to adjust first (different buckets, date range, "
        "   keyword filter)?\"\n"
        "\n"
        "5. When the user confirms:\n"
        "   - Use `select_entities_by_relationship` for each entity "
        "     type, passing the agreed-upon relationship_types list. "
        "     One call per entity type. Bulk-efficient.\n"
        "   - For more granular needs (date range, keyword) fall back "
        "     to `select_all_matching` with appropriate filters.\n"
        "   - For specific items, use `preview_entities` then "
        "     `select_entity` per item.\n"
        "\n"
        "6. After selection, summarise what's now in scope and offer "
        "   to `advance_to_data_room_setup`.\n"
        "\n"
        "When the user pushes back on a bucket (\"actually I want the "
        "comparables too, they're useful here\"), don't argue -- "
        "they have context you don't. Adjust the plan and proceed.\n"
    ),
    "data_room_setup": (
        "## OPENER (when the user's message is exactly `__opener__`)\n"
