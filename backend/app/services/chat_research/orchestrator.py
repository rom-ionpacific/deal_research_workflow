...
SYSTEM_PROMPTS: dict[str, str] = {
    "data_room_view": (
        ...
        "- `start_data_room_sweep(data_room_id, question)` + "
        "  `check_data_room_sweep(sweep_id)` -- LAST RESORT, not a "
        "  first choice: a systematic, exhaustive check of EVERY "
        "  readable document in the room against a specific "
        "  question. Only use this after ask_claude_room/ask_toltiq "
        "  have already come up empty or uncertain, or when the "
        "  user explicitly asks for an exhaustive/definitive check "
        "  ('search everything', 'are you sure nothing mentions "
        "  X'). Slower and more expensive than retrieval on purpose "
        "  -- it reads every document, not just the likely ones. "
        "  start_data_room_sweep only starts it (returns instantly); "
        "  call check_data_room_sweep repeatedly (it both advances "
        "  AND reports progress in one call) across turns until "
        "  status is 'complete'. Tell the user up front this will "
        "  take a few minutes for a large room. When complete with "
        "  hits, cite them with evidence. When complete with NO "
        "  hits, say the question was checked against every "
        "  readable document (docs_total) and none answered it -- "
        "  phrase as 'not found after an exhaustive check', never a "
        "  flat 'this does not exist' (some documents may be "
        "  unreadable -- scanned/OCR gaps, unsupported formats -- "
        "  and a systematic check can't rule those out; see the "
        "  room's Coverage tab for what's known-unreadable).\n"
