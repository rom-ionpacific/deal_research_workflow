...
def build_default_server():
    ...
    save_strategy_draft / finalize_strategy_agreement), and the interactive
    base-value agent's tools (get_base_value_context / set_base_value) —
    none of which exist on slack_registry itself, so Todd's Slack bot never
    gains write access or scenario-agent exposure. See chat_mcp_tools.py,
    chat_scenario_tools.py, and chat_base_value_tools.py.
